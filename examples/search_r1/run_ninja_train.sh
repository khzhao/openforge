...
JOB_DIR="${OPENFORGE_JOB_DIR:-/tmp/openforge-search-r1-$(date +%Y%m%d-%H%M%S)}"
...
MAX_VALIDATION_EXAMPLES="${OPENFORGE_MAX_VALIDATION_EXAMPLES:-64}"
...
openforge session start \
  --runtime-config examples/search_r1/runtime.yaml \
  >"$SESSION_LOG" 2>&1
