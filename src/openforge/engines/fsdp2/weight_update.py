...
from openforge.configs import DistributedPolicyWeightBucket, OpenForgeConfig
    ...
    def update_weights(self, *, policy_version: int) -> None:
        ...
        try:
            ...
            for name, param in self.model.state_dict().items():
                ...
                if (
                    bucket
                    and bucket_size + param_size >= self._buffer_size
                ):
...
class TensorWeightUpdater(WeightUpdater):
    ...
    def disconnect_rollout_engines(self) -> None:
        for group in self._created_groups:
            try:
                ...
                logger.debug("failed to destroy tensor sync process group", exc_info=True)
    ...
    def before_update(self) -> None:
        if self._rollout_worker is None or self.rank != self._gather_src_rank:
            return
        ray.get(
            self._rollout_worker.begin_tensor_weight_update.remote(
    ...
    def after_update(self) -> None:
        if self._rollout_worker is None or self.rank != self._gather_src_rank:
            return
        ray.get(self._rollout_worker.finish_tensor_weight_update.remote())
    ...
    def update_bucket_weights(
        ...
        from openforge.engines.sglang.serialization import serialize_named_tensors_for_sglang
        ...
        for payload_index in range(num_payloads):
            ray.get(
                self._rollout_worker.apply_tensor_weight_bucket.remote(
...
class DistributedWeightUpdater(WeightUpdater):
    ...
    def before_update(self) -> None:
        ...
        for index, worker in enumerate(self._rollout_workers):
            refs.append(
                worker.begin_distributed_weight_update.remote(
                    master_addr=self.master_addr,
                    master_port=master_port,
                    rank_offset=cumulative[index] + 1,
        ...
        self._process_group = init_custom_process_group(
            backend=self._backend,
            init_method=f"tcp://{self.master_addr}:{master_port}",
            world_size=world_size,
            rank=0,
            group_name=self._group_name,
    ...
    def after_update(self) -> None:
        ...
        if self._group_name is not None:
            refs = [
                worker.finish_distributed_weight_update.remote(
                    group_name=self._group_name
                )
    ...
    def update_bucket_weights(
        ...
        refs = [
            worker.apply_distributed_weight_bucket.remote(
                bucket=bucket,
                policy_version=self.weight_version,
                load_format="flattened_bucket",
                group_name=self._group_name,
