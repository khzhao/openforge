        ...
        if off_experts != -1:
            ...
            for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
                ...
                grad_input_ptrs = grad_input_ptr + (
                    (offs_token[:, None] // top_k) * stride_gim
                    + curr_offs_k[None, :] * stride_gik
                )
                grad_input_mask = token_mask[:, None] & (curr_offs_k[None, :] < K)
                tl.atomic_add(
                    grad_input_ptrs, contribution.to(compute_type), mask=grad_input_mask
                )
...
def fused_moe_backward_weight_kernel(
    ...
    offs_token = offs_token.to(tl.int64)
    token_mask = (offs_token_id < num_tokens_post_padded) & (
    ...
    if MUL_ROUTED_WEIGHT:
        moe_weight = tl.load(
            topk_weights_ptr + offs_token_clamped, mask=token_mask, other=0.0
        )
    ...
    for k_block in range(tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        tl.atomic_add(
            grad_weight_ptrs,
            grad_w_contribution.to(compute_type),
            mask=grad_weight_mask,
        )
    ...
    if pid * BLOCK_SIZE_M < num_tokens_post_padded:
        ...
        if off_experts != -1:
            ...
            for n in range(0, tl.cdiv(N, BLOCK_SIZE_N)):
                ...
                grad_output_ptrs = grad_output_ptr + (
                    offs_token_clamped[:, None] * stride_gom
                    + curr_offs_n[None, :] * stride_gon
                )
                grad_out = tl.load(
                    grad_output_ptrs,
