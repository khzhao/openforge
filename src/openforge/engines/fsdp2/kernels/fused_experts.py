...
class GateUpProjFunction(torch.autograd.Function):
    """Custom autograd function for fused gate + up projection in MoE layers."""
    ...
    def backward(ctx, grad_output):
        ...
        Args:
            ctx: Autograd context with saved tensors and config from forward
...
class SiluAndMulFunction(torch.autograd.Function):
    """Custom autograd function for fused SiLU activation and element-wise multiply."""
...
class DownProjFunction(torch.autograd.Function):
    """Custom autograd function for fused down projection in MoE layers."""
    ...
    def backward(ctx, grad_output):
        ...
        for chunk in range((num_tokens // CHUNK_SIZE) + 1):
            ...
            # Accumulate gradients
            grad_intermediate_cache2[begin_chunk_idx * topk : end_chunk_idx * topk] = (
...
class MoeSumReduceFunction(torch.autograd.Function):
    """Custom autograd function for summing MoE expert outputs with routing weights."""
